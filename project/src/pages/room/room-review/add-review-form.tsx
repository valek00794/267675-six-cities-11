...
function AddReviewForm(): JSX.Element {
  ...
  /*const fieldChangeCommentHandle = (evt: { target: { value: string; name: string } }) => {
  ...
  /*const fieldChangeHandle = (evt: { target: { value: NewComment[keyof NewComment]; name: string } }) => {
    if (evt.target.name === 'comment' && evt.target.value.toString().length > MIN_COMMENT_LENGTH) {
      if (evt.target.name === 'rating' && evt.target.value > 0) {
        setEnableButtnon(false);
      } else {
        setEnableButtnon(true);
      }
